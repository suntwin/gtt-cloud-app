...
gtt_endpoints = {
    "1M": "https://api.marketinout.com/run/screen?key=703211892be14153",
    "3M": "https://api.marketinout.com/run/screen?key=259130e049674245",
    "6M": "https://api.marketinout.com/run/screen?key=e235ef898f6342f7"
}
...
weekly_endpoint = "https://api.marketinout.com/run/screen?key=bca3e0acee5d40fd"
...
SECTOR_FILE = os.path.join(PROJECT_DIR, "TradingView", "Symbols_ASX.csv")
...
def fetch_gtt_scan(url, name):
    try:
        ...
        if response.status_code == 200 and response.text.strip():
            ...
            df['Symbol'] = df['Symbol'].str.upper().str.replace('.AX', '', regex=False)
...
def main():
    ...
    """
    st.title("⚡ GTT Trade Generator (ASX)")
    ...
    if file_age is not None:
        ...
        st.sidebar.error("⚠️ Symbols_ASX.csv not found!")
    ...
    with tab1:
        if 'gtt_base_df' in st.session_state and st.session_state.gtt_base_df is not None:
            ...
            if not sorted_df.empty and 'Symbol' in sorted_df.columns and 'Tier' in sorted_df.columns:
                ...
                tier_a_tv_string = ",".join([f"asx:{s}" for s in tier_a_symbols])

                tier_b_df = sorted_df[sorted_df['Tier'] == '🟡 B']
                tier_b_symbols = tier_b_df['Symbol'].dropna().unique().tolist()
                tier_b_tv_string = ",".join([f"asx:{s}" for s in tier_b_symbols])
