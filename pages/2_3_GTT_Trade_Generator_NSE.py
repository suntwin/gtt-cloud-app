...
def main():
    ...
    with tab1:
        if 'gtt_base_df' in st.session_state and st.session_state.gtt_base_df is not None:
            ...
            st.session_state.gtt_scored_df = actionable_df.copy()  # Saved for Tab 3 to use
    ...
    # ════════════════════════════════════════════════════════════════
    # TAB 3 — SAVED BREAKOUTS (MULTI-DAY WATCHLIST VIA SUPABASE)
    # ════════════════════════════════════════════════════════════════
    with tab3:
        st.subheader("Saved Exceptional Breakouts")
        st.caption("Tracks your saved tickers and updates their live parameters every time you run the scanner.")
        ...
        with col1:
            new_ticker = st.text_input("Enter Ticker to Track (e.g., BTC, SOL):",
        ...
        if 'gtt_scored_df' in st.session_state and st.session_state.gtt_scored_df is not None and saved_breakouts:
            ...
            if '_chg_percentclose_hourly' in merged_df.columns:
                gb3.configure_column('_chg_percentclose_hourly', minWidth=80, maxWidth=110)
            ...
            AgGrid(safe_merged_df, gridOptions=gb3.build(),
                   update_mode=GridUpdateMode.MODEL_CHANGED,
                   fit_columns_on_grid_load=False,
                   height=600, theme='streamlit', key='saved_breakouts_grid',
                   allow_unsafe_jscode=True)
