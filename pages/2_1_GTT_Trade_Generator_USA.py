...
MARKET_CFG = MARKETS["USA"]   # user us_user, New York hours, bare TradingView symbols
...
def load_column_prefs(table_key):
    ...
    try:
        r = supabase.table("column_prefs").select("visible_columns").eq("table_key", table_key).eq("user_id", "us_user").execute()
...
def save_column_prefs(table_key, cols):
    ...
    try:
        ex = supabase.table("column_prefs").select("id").eq("table_key", table_key).eq("user_id", "us_user").execute()
        if ex.data:
...
def load_scoring_prefs(scanner_type):
    ...
    if not supabase:
        ...
        return {}
    try:
        r = supabase.table("scoring_prefs").select("config").eq("user_id", "us_user").eq("scanner_type", scanner_type).execute()
...
def save_scoring_prefs(prefs, scanner_type):
    ...
    if not supabase:
        ...
        return
    try:
        ex = supabase.table("scoring_prefs").select("id").eq("user_id", "us_user").eq("scanner_type", scanner_type).execute()
        if ex.data:
            supabase.table("scoring_prefs").update({"config": prefs}).eq("user_id", "us_user").eq("scanner_type", scanner_type).execute()
        else:
            supabase.table("scoring_prefs").insert({"user_id": "us_user", "scanner_type": scanner_type, "config": prefs}).execute()
    except Exception as e:
        try:
            with open(lf, 'w') as f: json.dump(prefs, f, indent=2)
        except: pass
...
def main():
    ...
    else: st.sidebar.error("Symbols_USA.csv not found!")
    ...
    saved_prefs = load_scoring_prefs("USA")
        ...
        with st.spinner("Fetching and merging multi-timeframe scans..."):
            ...
            if df_1m is not None and not df_1m.empty:
                ...
                if wdf is not None and not wdf.empty:
                    ...
                    ws = wdf[['Symbol','Pctof10wkhigh','Weeklyclose_chg_pct','Tightcloses_of4','Insidebars_of8','Dist_wema10_pct']].rename(columns={
                        'Pctof10wkhigh':'W_PctOf10wkHigh','Weeklyclose_chg_pct':'W_CloseChg_Pct','Tightcloses_of4':'W_TightCloses_10w',
    ...
    with tab1:
        if 'gtt_base_df' in st.session_state and st.session_state.gtt_base_df is not None:
            ...
            with st.expander("Choose visible columns (saved as your default)"):
                ...
                _saved = get_persisted_columns('us_main_table_v2', amc, mtdh)
                ...
                if st.button("Save as default", key="save_cols"): save_column_prefs('us_main_table_v2',
            ...
            if not sdf.empty and 'Symbol' in sdf.columns:
                ...
                _tv = lambda xs: ",".join(tv_symbol(x, MARKET_CFG) for x in xs)
                if scan_mode == "Post Breakout":
